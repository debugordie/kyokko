...
//    tb_au200: Testbench for Xilinx Alveo U200 simulation
// ----------------------------------------------------------------------
...
`ifndef NO_LOOPBACK
   ...
   assign {QSFP1_RXP, QSFP0_RXP} = {QSFP1_TXP, QSFP0_TXP};
   assign {QSFP1_RXN, QSFP0_RXN} = {QSFP1_TXN, QSFP0_TXN};
`endif
   ...
   for (ch=0; ch<NumChB; ch=ch+1) begin : ch_up_gen
      ...
      always @ (posedge uut.AURORA_CLK[ch]) begin
         ...
         if (~CH_UP_R & uut.CH_UP[ch])  
           $display("%m AU200 channel %2d up at %f", ch, $realtime);

         if (CH_UP_R & ~uut.CH_UP[ch])  
           $display("%m AU200 channel %2d down at %f", ch, $realtime);
      end
   end
