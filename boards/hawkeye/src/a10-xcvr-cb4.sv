...
//    a10_xcvr_cb4: Channel-bonded Quad Arria 10 GX transceiver wrapper
//    c10_phy_bitorder: Bit order reverser for Intel GX transceivers
// ----------------------------------------------------------------------

`default_nettype none

module a10_xcvr_cb4 # ( parameter NumCh=4 )
   ( input wire                 RST, // RST somehow
     ...
     output wire                TX_USRCLK, RX_USRCLK,
   ...
   atx_5g_4cb atxpll
     ( .pll_powerdown     (PLL_PD),           // I
       ...
       .pll_cal_busy      (),                 // O
       .tx_bonding_clocks (TX_BONDING_CLK) ); // O [5:0]
   ...
   wire [NumCh-1:0][63:0] RX_DATAi, TX_DATAi;
   wire [NumCh-1:0][1:0]  RX_CTRLi, TX_CTRLi;

   phy_10g_4cb phy0 
     ( .tx_analogreset          (TX_ARST),                 // I [4]
       .tx_digitalreset         (TX_DRST),                 // I [4]
       .rx_analogreset          (RX_ARST),                 // I [4]
       .rx_digitalreset         (RX_DRST),                 // I [4]
       .tx_cal_busy             (TX_CALBUSY),              // O [4]
       .rx_cal_busy             (RX_CALBUSY),              // O [4]
       .tx_bonding_clocks       ({NumCh{TX_BONDING_CLK}}), // O [24]
       .rx_cdr_refclk0          (REFCLK644P),              // I [1]
       .tx_serial_data          (SFP_TXP),                 // O [4]
       .rx_serial_data          (SFP_RXP),                 // I [4]
       .rx_is_lockedtoref       (),                        // O [4]
       .rx_is_lockedtodata      (RX_LOCKED),               // O [4]
       .tx_coreclkin            ({NumCh{TX_USRCLK}}),      // I [4]
       .rx_coreclkin            ({NumCh{RX_USRCLK}}),      // I [4]
       .tx_clkout               (),                        // O [4]
       .rx_clkout               (),                        // O [4]
       .tx_pma_div_clkout       (TX_USRCLK),               // O [4]
       .rx_pma_div_clkout       (RX_USRCLK),               // O [4]
       .tx_parallel_data        (TX_DATAi),                // I [256]
       .tx_control              (TX_CTRLi),                // I [8]
       .unused_tx_parallel_data (),                        // I [256]
       .unused_tx_control       (),                        // I [64]
       .rx_parallel_data        (RX_DATAi),                // O [256]
       .rx_control              (RX_CTRLi),                // O [8]
       .unused_rx_parallel_data (),                        // O [256]
       .unused_rx_control       (),                        // O [72]
       .rx_bitslip              (RX_BITSLIP),              // I [4]
       .tx_enh_data_valid       ({NumCh{TX_VALID[0]}}),    // I [4]
       .tx_enh_fifo_full        (),                        // O [4]
       .tx_enh_fifo_empty       (),                        // O [4]
       .rx_enh_fifo_rd_en       ({NumCh{1'b1}}),           // I [4]
       .rx_enh_data_valid       (),                        // O [4]
       .rx_enh_fifo_full        (),                        // O [4]
       .rx_enh_fifo_empty       ()                         // O [4]
       );
